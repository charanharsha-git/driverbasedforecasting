...
def forecast():
    if request.method == 'POST':
        ...
        product_name = request.form.getlist('productFamily')[0]
        train, actuals, prediction1 = prediction_fn(store_no, forecast_days, promotion)


        # Pass the dataframe to the template for rendering
        return render_template('plot.html', dataframe=prediction1)
